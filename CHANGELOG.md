...
- Passive encoding-suspect detection: `slice` and `outline` receipts gain an `encoding_suspects` object (plus a one-line human note) when the decoded file carries proof-grade mojibake — `double_encoded` counts character runs whose CP1252 byte round-trip re-decodes as valid multi-byte UTF-8 (the garbled multi-character form an em-dash or accented letter takes when UTF-8 is re-read as CP1252, with the recovered character named in `sample`), `replacement_chars` counts U+FFFD (a lossy decode already happened, possibly this read), and `c1_controls` counts raw C1-range characters. A 2-byte run is trusted only with a Latin-1 lead or when it clusters, so legitimate accented text never trips it (validated at 0% false positives on a 6.8k-file multilingual corpus). `capture` receipts carry the double-encoded count for child output, catching UTF-8 written through a CP1252 boundary (the classic PowerShell 5.1 hazard) at the moment it happens. The field is omitted entirely when nothing is found, so clean files cost nothing; detection never fails a command.
