...
- Passive encoding-suspect detection: `slice` and `outline` receipts gain an `encoding_suspects` object (plus a one-line human note) when the decoded file carries proof-grade mojibake — `double_encoded` counts character runs whose CP1252 byte round-trip re-decodes as valid multi-byte UTF-8 (`â€”` for `—`, with the repair named in `sample`), `replacement_chars` counts U+FFFD (a lossy decode already happened, possibly this read), and `c1_controls` counts raw C1-range characters. `capture` receipts carry the double-encoded count for child output, catching UTF-8 written through a CP1252 boundary (the classic PowerShell 5.1 hazard) at the moment it happens. The field is omitted entirely when nothing is found, so clean files cost nothing; detection is a byte-level round-trip proof, not a pattern list, and it never fails a command.
